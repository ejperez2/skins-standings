...
player_scores <- draft_data %>%
  ...
  mutate(Projection_Average = rowMeans(select(., `Weighted Average`, `Vegas Consensus`, `TeamRankings`, `ESPN BPI`, `CBS Sports`, `BB-Ref`))) %>%
...
kable(player_scores_with_avg, align = c("l", "r", "r", "r", "r", "r", "r", "r", "r", "r", "r")) %>%
...
standings_table <- draft_data %>%
  ...
  rowwise() %>%
  mutate(
    `Projection Average` = mean(c(`Weighted Average`, `Vegas Consensus`, `TeamRankings`, `ESPN BPI`, `CBS Sports`, `BB-Ref`), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  select(
    team,
...
for(i in seq_along(player_order)) {
  ...
  table_html <- kable(
    player_data,
    align = c("c", "c", "r", "r", "r", "r", "r", "r", "r", "r", "r", "r"),
    escape = FALSE
  ) %>%
    kable_styling(
...
team_projections_table <- df_final_projections %>%
  ...
  rowwise() %>%
  mutate(
    `Projection Average` = mean(c(weighted_average, vegas_consensus, tr_skins, espn_skins, cbs_skins, br_skins), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  select(
    Team = team,
...
kable(team_projections_table, align = c("l", "l", "c", "r", "r", "r", "r", "r", "r", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE, position = "center") %>%
  row_spec(0, bold = TRUE, color = "white", background = "#444444") %>%
  column_spec(1, bold = TRUE) %>%
