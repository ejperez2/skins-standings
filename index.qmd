...
# A: Get current NBA standings using hoopR with better error handling for CI/CD
nba_standings <- tryCatch({
  # Attempt to use hoopR's standings function
  standings_raw <- hoopR::nba_leaguestandingsv3()
  ...
  # Fallback: Call the API directly if hoopR function fails
  message("hoopR function failed, using direct API call")
  ...
  tryCatch({
    ...
    standings_df <- as.data.frame(json_data$resultSets$rowSet[[1]], stringsAsFactors = FALSE)
    colnames(standings_df) <- json_data$resultSets$headers[[1]]
    ...
    # Final fallback: return 0-0 for all teams if everything fails
    message("All attempts failed, using 0-0 records")
    tibble(
