...
df_skins <- df_vegas %>%
  ...
  mutate(
    ...
    # Calculate composite projection as mean of adjusted totals (this is projected WINS)
    vegas_proj_wins = mean(c(mgm_adj, draftkings_adj, caesars_adj, betrivers_adj, hardrock_adj, fanduel_adj, espnbet_adj), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(
    vegas_proj_losses = 82 - vegas_proj_wins
  )
...
cbs_proj <- bind_rows(eastern_conf, western_conf) %>%
  ...
  mutate(
    cbs_proj_w = as.numeric(cbs_proj_w),
    cbs_proj_l = 82 - cbs_proj_w
  )
...
tr_cleaned <- tr_cleaned %>%
  ...
  select(team, tr_proj_w, tr_proj_l)
...
df_final_projections <- all_teams %>%
  ...
  left_join(df_skins %>% select(team, vegas_proj_wins, vegas_proj_losses), by = "team") %>%
  left_join(tr_cleaned, by = "team") %>%
  left_join(espn_cleaned %>% select(team, espn_proj_w, espn_proj_l), by = "team") %>%
  left_join(cbs_proj, by = "team") %>%
  left_join(nba_standings, by = "team") %>%
  mutate(
    # Calculate skins for each projection source based on draft pick
    vegas_consensus = if_else(skins_pick == "W", vegas_proj_wins, vegas_proj_losses),
    tr_skins = if_else(skins_pick == "W", tr_proj_w, tr_proj_l),
    espn_skins = if_else(skins_pick == "W", espn_proj_w, espn_proj_l),
    cbs_skins = if_else(skins_pick == "W", cbs_proj_w, cbs_proj_l),
    # Calculate weighted average including all projection sources
    weighted_average = if_else(
      !is.na(vegas_consensus),
      (vegas_consensus * 0.25) + (tr_skins * 0.08) + (espn_skins * 0.47) + (cbs_skins * 0.20),
      (tr_skins * 0.15) + (espn_skins * 0.60) + (cbs_skins * 0.25)  # Fallback formula without Vegas
    ),
...
# Prepare current team vegas data (store the projected wins/losses before conversion)
current_team_vegas <- df_final_projections %>%
  ...
  select(team, vegas_consensus, date)
...
# For teams with NA vegas_consensus, find the last non-NA value
df_final_projections <- df_final_projections %>%
  rowwise() %>%
  mutate(
    vegas_consensus = if_else(
      is.na(vegas_consensus),
      {
        # Find last non-NA value for this team from history
        current_team <- team
        last_valid <- team_vegas_history %>%
          filter(team == current_team, !is.na(vegas_consensus), date < today) %>%
          arrange(desc(date)) %>%
          slice(1) %>%
          pull(vegas_consensus)
...
# Recalculate weighted average with updated vegas_consensus
df_final_projections <- df_final_projections %>%
  mutate(
    weighted_average = if_else(
      !is.na(vegas_consensus),
      (vegas_consensus * 0.31) + (tr_skins * 0.10) + (espn_skins * 0.59),
      (tr_skins * 0.25) + (espn_skins * 0.75)
    )
  )
...
kable(player_scores_display, align = c("l", "r", "r", "r", "r", "r", "r", "r", "r", "r")) %>%
...
kable(team_projections_table, align = c("l", "l", "c", "r", "r", "r", "r", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE, position = "center") %>%
  row_spec(0, bold = TRUE, color = "white", background = "#444444") %>%
  column_spec(1, bold = TRUE) %>%
