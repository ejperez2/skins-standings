...
df_skins <- df_vegas %>%
  ...
  ungroup() %>%
  select(team, composite_projection) # **MODIFIED**: Only select the raw projection
...
# **MODIFIED**: Only select raw W/L projections
...
espn_cleaned <- espn_proj %>%
  ...
  select(team, espn_proj_w, espn_proj_l)
...
# **MODIFIED**: Apply standardization to the cleaned tables
tr_cleaned <- tr_cleaned %>%
  standardize_team_names("team")

espn_cleaned <- espn_cleaned %>% 
  standardize_team_names("team")
...
# **MODIFIED**: Join raw projection columns, THEN calculate skins values
df_final_projections <- all_teams %>%
  left_join(draft_data %>% select(team, abbr, skins_pick), by = "team") %>%
  left_join(df_skins, by = "team") %>% # Brings in composite_projection
  left_join(tr_cleaned, by = "team") %>% # Brings in tr_proj_w, tr_proj_l
  left_join(espn_cleaned, by = "team") %>% # Brings in espn_proj_w, espn_proj_l
  left_join(nba_standings, by = "team") %>%
  mutate(
    # --- NEW: Calculate skins values based on hardcoded skins_pick ---
    vegas_consensus = if_else(skins_pick == "W", composite_projection, 82 - composite_projection),
...
# Prepare current team vegas data
current_team_vegas <- df_final_projections %>%
  select(team, vegas_consensus) %>%
  mutate(date = today)
...
kable(player_scores_display, align = c("l", "r", "r", "r", "r", "r", "r", "r", "r")) %>%
...
kable(team_projections_table, align = c("l", "l", "c", "r", "r", "r", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE, position = "center") %>%
  row_spec(0, bold = TRUE, color = "white", background = "#444444") %>%
  column_spec(1, bold = TRUE) %>%
